...
2. **Storage** -- All messages, threads, labels, contacts, calendar events, and AI results stored in local SQLite (32 tables) with FTS5 full-text indexing.
...
SQLite via Tauri SQL plugin. 28 migrations, 32 tables total.
...
- **Flatpak**: A Flatpak manifest (`com.velomail.app.yml`) defines the sandbox environment, leveraging the GNOME 50 runtime and the Node 24 SDK extension (Rust comes from rustup during the build). Local builds are streamlined via an npm script (`npm run flatpak`) which uses `flatpak-builder` while excluding host-specific artifacts to ensure reproducible sandboxed builds. The release bundle is built with `--runtime-repo` pointing at Flathub so a machine without the runtime can fetch it; the packaging job can be dispatched on any branch to prove a runtime bump before a release (SPEC-233).
