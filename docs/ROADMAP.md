...
Enhancement wave 1 is complete on main (#80/#82 auto reminders, #87 custom split-inbox tabs, #90 Instant Intro, #92 the speed budget), the follow-up reminder insert is repaired (#88), and its Tier 2 index migration has landed (#94, migration 29). Next: **wave 2 from docs/ROADMAP.md §4 in order — Share Availability in the composer (brief approved, PR #95; scoped to Google-backed calendars, CalDAV disabled because recurring events are never expanded), then Instant Event, then one-line list summaries** — briefs first, one PR per item. Review legs: Gemini 3.8 Flash High via agy AND Grok 4.6 via the grok CLI, diffs from committed SHAs; verify every finding against source before adopting; and review every fix you write, not just the change — on #92 five rounds each found a real defect in the previous round's fix. You own commits, pushes, PRs and merges. No dependency beyond the ones already approved; ask before any other.
