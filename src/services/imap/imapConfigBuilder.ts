...
export function buildSmtpConfig(
  ...
  const authMethod = mapAuthMethod(account.auth_method);
  // SPEC-252 REQ-1.4: the account's own SMTP credentials when it has them,
  // per field, else the IMAP ones — so every account saved before migration
  // 28 produces exactly the config it did before.
  const password =
    authMethod === "oauth2" && accessToken
      ? accessToken
