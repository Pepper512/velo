...
2. **Service layer** (`src/services/`): All business logic. Plain async functions (not classes, except `GmailClient`).
   ...
   - `ai/` — `aiService.ts` provides thread summaries, smart replies, AI compose, text transform, auto-categorization, smart label classification, and task extraction. `providerManager.ts` manages six providers (`claudeProvider`, `openaiProvider`, `geminiProvider`, `ollamaProvider`, `copilotProvider`, `xaiProvider`); the OpenAI-compatible ones share `providers/openAiCompatible.ts`. `askInbox.ts` enables natural language inbox queries. `categorizationManager.ts` auto-sorts threads into Primary/Updates/Promotions/Social/Newsletters. `writingStyleService.ts` analyzes user writing style from sent emails and generates auto-draft replies. `taskExtraction.ts` extracts tasks from email threads via AI. `errors.ts` and `types.ts` define shared AI types. Results cached locally via `db/aiCache.ts`.
...
163 test files across services (90), components (35), utils (17), stores (9), constants (4), hooks (3), config (2), router (2), test (1). Both numbers are measured by `npm run docs:check`, not maintained by hand.
