...
1. **Rust backend** (`src-tauri/`): System tray, minimize-to-tray (hide on close), splash screen, OAuth localhost server (port 17248, PKCE), single-instance enforcement, autostart support, IMAP/SMTP client modules. Tauri commands: `start_oauth_server`, `close_splashscreen`, `set_tray_tooltip`, `open_devtools`, plus 11 IMAP commands (`imap_test_connection`, `imap_list_folders`, `imap_fetch_messages`, `imap_fetch_new_uids`, `imap_fetch_message_body`, `imap_set_flags`, `imap_move_messages`, `imap_delete_messages`, `imap_get_folder_status`, `imap_fetch_attachment`, `imap_append_message`) and 2 SMTP commands (`smtp_send_email`, `smtp_test_connection`), plus 5 transaction commands (`db_tx_begin`, `db_tx_execute`, `db_tx_select`, `db_tx_commit`, `db_tx_rollback` — `src-tauri/src/db/tx.rs`, one pinned connection per transaction, SPEC-240), plus `ai_fetch` (`src-tauri/src/ai_fetch.rs`, SPEC-209: the one door for a user-configured OpenAI-compatible endpoint — `https:` to any host or `http:` to loopback only, no redirect followed, GET/POST, four request headers forwarded, 8 MiB body cap, 120 s timeout), plus `connection_test_cancel` (`src-tauri/src/connection_tests.rs`, SPEC-204: `imap_test_connection`/`smtp_test_connection` take an optional `testId` and run as an abortable task; the cancel command aborts it — the add-account form's Cancel). Rust IMAP uses `async-imap` + `mail-parser`, SMTP uses `lettre`. Plugins: sql (SQLite), notification, opener, log, dialog, fs, http, single-instance, autostart, deep-link (`mailto:` scheme), global-shortcut. Windows-specific: sets AUMID for proper notification identity.
...
2. **Service layer** (`src/services/`): All business logic. Plain async functions (not classes, except `GmailClient`).
   ...
   - `ai/` — `aiService.ts` provides thread summaries, smart replies, AI compose, text transform, auto-categorization, smart label classification, and task extraction. `providerManager.ts` manages seven providers (`claudeProvider`, `openaiProvider`, `geminiProvider`, `ollamaProvider`, `copilotProvider`, `xaiProvider`, `customProvider`); the OpenAI-compatible ones share `providers/openAiCompatible.ts`. `customProvider` takes the user's base URL and reaches it through `rustFetch.ts` → the Rust `ai_fetch` command (the CSP cannot admit a runtime host; the http plugin's scope enforces no scheme/host rule). `askInbox.ts` enables natural language inbox queries. `categorizationManager.ts` auto-sorts threads into Primary/Updates/Promotions/Social/Newsletters. `writingStyleService.ts` analyzes user writing style from sent emails and generates auto-draft replies. `taskExtraction.ts` extracts tasks from email threads via AI. `errors.ts` and `types.ts` define shared AI types. Results cached locally via `db/aiCache.ts`.
...
181 test files across services (101), components (40), utils (18), stores (9), constants (4), hooks (3), config (3), router (2), test (1). Both numbers are measured by `npm run docs:check`, not maintained by hand.
...
- **SMTP credentials (SPEC-252)**: optional `smtp_username` and encrypted `smtp_password` on accounts (migration 28); `buildSmtpConfig` uses them per field and falls back to the IMAP username/password when null, so pre-28 accounts are unchanged. The add-account form's SMTP test and save both go through `resolveSmtpCredentials` — never derive the two separately (#252 was an identical ternary)
...
- **CSP**: Allows connections to googleapis.com, anthropic.com, openai.com, generativelanguage.googleapis.com, gravatar.com, googleusercontent.com. `img-src` is `'self' data: https:` (SPEC-197): the remote-image block is the sanitizer's job (`stripRemoteImages`, per-sender allowlist), not the CSP's — never add `http:`. `src/config/tauriConfig.test.ts` pins every directive exactly
...
- **AI providers**: API keys stored in SQLite settings table. Provider selected per-feature in settings. Results cached in `ai_cache` table. The **custom OpenAI-compatible endpoint** (SPEC-209) is the only provider whose host is not in the CSP: its requests go through `ai_fetch` in Rust, which allows `https:` anywhere or `http:` to loopback only and never follows a redirect — never route it through the webview's `fetch` or the http plugin, and never widen `connect-src` for it
